...
// Na v1, achar um cliente ou um evento exige ir na página certa e filtrar. Aqui
// qualquer coisa é alcançável de qualquer tela, digitando.
//
// FASE A: os resultados levam para telas de leitura da v2 (ou para a v1 quando
// a ação só existe lá). Os eventos são carregados uma vez, quando a busca abre
// pela primeira vez.
...
const NAV_ROWS: Row[] = [
  ...
  { id: 'n3', group: 'Ir para', icon: PartyPopper, label: 'Eventos', href: '/v2/eventos' },
...
export function CommandPalette({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const rows = useMemo<Row[]>(() => {
    ...
    const eventRows: Row[] = events.map((e) => ({
      ...
      icon: CalendarDays,
      label: e.title,
      hint: `${format(e.start, "d 'de' MMM", { locale: ptBR })} · ${e.space.name} · ${brl(e.value)}`,
      href: `/v2/eventos?id=${e.id}`,
      color: e.space.color,
    }))

    const clientRows: Row[] = events
      .filter((e) => e.client)
      .map((e) => ({
        id: `c${e.id}`,
        ...
        icon: Users,
        label: e.client!.name,
        hint: [e.client!.phone, e.client!.city].filter(Boolean).join(' · '),
        href: `/v2/eventos?id=${e.id}`,
      }))
                ...
                <div key={row.id}>
                  ...
                  >
                    <span
                      ...
                      style={{ background: row.color ? `${row.color}1f` : 'var(--v2-surface-3)' }}
                    >
